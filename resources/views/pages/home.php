...
<section class="hero">
    <div class="hero-grid">
        ...
        <a href="<?= \App\Core\App::get('config')['url'] ?>/article?id=<?= $main_hero['id'] ?>" class="hero-card">
            <img src="<?= $main_hero['image_url'] ? htmlspecialchars($main_hero['image_url']) : 'https://placehold.co/800x400?text=News' ?>" alt="Hero Image">
            <div class="hero-overlay">
                <span class="category-badge"><?= htmlspecialchars($main_hero['category_name'] ?? 'General') ?></span>
                <h2><?= htmlspecialchars($main_hero['title']) ?></h2>
                <p><?= htmlspecialchars($main_hero['summary'] ?? substr($main_hero['title'], 0, 60)) ?></p>
            </div>
        </a>
        ...
        <div style="display:flex; flex-direction:column; gap:1rem;">
            ...
            <a href="<?= \App\Core\App::get('config')['url'] ?>/article?id=<?= $side_hero['id'] ?>" class="hero-card small">
                <img src="<?= $side_hero['image_url'] ? htmlspecialchars($side_hero['image_url']) : 'https://placehold.co/400x200?text=News' ?>" alt="Side Hero">
                <div class="hero-overlay">
                    <span class="category-badge"><?= htmlspecialchars($side_hero['category_name'] ?? 'General') ?></span>
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-lg-4 mb-4">
            <div class="card h-100 shadow-sm border-0 article-card" style="transition: transform 0.2s;">
                <a href="<?= \App\Core\App::get('config')['url'] ?>/article?id=<?= $article['id'] ?>" style="text-decoration:none; color:inherit;">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text text-muted mb-3"><?= htmlspecialchars($article['summary'] ?? substr($article['title'], 0, 70)) ?></p>
...
<?php foreach ($categories as $cat): ?>
    <?php if (isset($category_articles[$cat['id']])): ?>
    <section class="category-row">
        <div class="section-title">
            <h3><?= htmlspecialchars($cat['name']) ?></h3>
            <a href="<?= \App\Core\App::get('config')['url'] ?>/category?id=<?= $cat['id'] ?>" class="view-all">View All <i class="fas fa-chevron-right"></i></a>
        </div>
