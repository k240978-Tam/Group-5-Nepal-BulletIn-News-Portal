...
if ($article_id > 0) {
    try {
        $stmt = $pdo->prepare("SELECT t.name FROM tags t JOIN article_tags at2 ON t.id = at2.tag_id WHERE at2.article_id = ?");
        $stmt->execute([$article_id]);
...
<form id="aForm" action="/newsportal/actions/process_article.php" method="POST" enctype="multipart/form-data">
...
<div>
    ...
    <div class="ed-card">
        ...
        <div class="fg">
            ...
            <select name="article_type" class="fc">
                <option value="standard"    <?= ($article['article_type'] ?? 'standard')=='standard'   ? 'selected':'' ?>>📰 Standard</option>
                <option value="opinion"     <?= ($article['article_type'] ?? '')=='opinion'             ? 'selected':'' ?>>💬 Opinion / Editorial</option>
                <option value="interview"   <?= ($article['article_type'] ?? '')=='interview'           ? 'selected':'' ?>>🎤 Interview</option>
                <option value="photo_essay" <?= ($article['article_type'] ?? '')=='photo_essay'         ? 'selected':'' ?>>📷 Photo Essay</option>
                <option value="breaking"    <?= ($article['article_type'] ?? '')=='breaking'            ? 'selected':'' ?>>🔴 Breaking News</option>
            </select>
        </div>
        <div class="fg" style="margin:0">
            <label>Language</label>
            <select name="language" class="fc">
                <option value="en"        <?= ($article['language'] ?? 'en')=='en'         ? 'selected':'' ?>>🇬🇧 English</option>
                <option value="ne"        <?= ($article['language'] ?? '')=='ne'            ? 'selected':'' ?>>🇳🇵 Nepali (नेपाली)</option>
                <option value="bilingual" <?= ($article['language'] ?? '')=='bilingual'     ? 'selected':'' ?>>🌐 Bilingual</option>
            </select>
        </div>
    </div>
    ...
    <div class="ed-card">
        ...
        <div class="fg">
            ...
            <div class="img-pos-grid">
                <div class="ipos <?= ($article['image_position']??'center')=='left'  ?'on':'' ?>" onclick="setPos('left',this)"><i class="fas fa-align-left"></i>Left</div>
                <div class="ipos <?= ($article['image_position']??'center')=='center'?'on':'' ?>" onclick="setPos('center',this)"><i class="fas fa-align-center"></i>Center</div>
                <div class="ipos <?= ($article['image_position']??'center')=='right' ?'on':'' ?>" onclick="setPos('right',this)"><i class="fas fa-align-right"></i>Right</div>
                <div class="ipos <?= ($article['image_position']??'center')=='full'  ?'on':'' ?>" onclick="setPos('full',this)"><i class="fas fa-expand"></i>Full</div>
            </div>
        </div>
        <div class="fg" style="margin:0">
            <label>Image Caption</label>
            <input type="text" name="image_caption" class="fc" placeholder="Photo: Source / Agency" value="<?= htmlspecialchars($article['image_caption'] ?? '') ?>">
        </div>
    </div>
    ...
    <div class="ed-card">
        ...
        <div class="swatches">
            ...
            <div class="sw <?= ($article['article_color']??'#c0392b')===$sw?'on':'' ?>" style="background:<?=$sw?>" onclick="setCol('<?=$sw?>',this)" title="<?=$sw?>"></div>
            <?php endforeach; ?>
        </div>
    </div>
